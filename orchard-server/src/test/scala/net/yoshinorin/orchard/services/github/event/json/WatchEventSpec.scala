...
class WatchEventSpec extends FunSuite {
  ...
  test("getConvertedCaseClass method should return none") {
    val json = """Not a JSON"""
    assert(WatchEvent(eventInstance.event.get, parse(json).getOrElse(Json.Null)).getConvertedCaseClass.isEmpty)
  }

  test("insert method is callable") {
    val json = """Not a JSON"""
    assert(WatchEvent(eventInstance.event.get, parse(json).getOrElse(Json.Null)).insert.isInstanceOf[Unit])
  }
